...
export default function Home() {
  ...
  async function load() {
    ...
    const [{ data: m }, { data: l }] = await Promise.all([
      supabase.from("matches").select("id,fifa_match_number"),
      supabase
        .from("listings")
        .select("id,match_id,status,expires_at,archived_at")
        .eq("status", "active")
        .is("archived_at", null),
    ]);
      ...
      <div style={{ background: "transparent", borderBottom: `1px solid ${C.border}` }}>
        <div style={{ ...W, paddingTop: "44px", paddingBottom: "40px" }}>
          ...
          >
            ...
            >
              {[
                { val: loading ? "--" : "2", lbl: isHe ? "קטגוריות" : "Categories", color: C.usa },
                { val: loading ? "--" : activeCount, lbl: isHe ? "מודעות פעילות" : "Live listings", color: C.text, live: true },
                { val: loading ? "--" : `${matches.length || 0}`,
                  lbl: isHe ? "אירועי ספורט" : "Sports events", color: C.mexico },
              ].map((s, i) => (
          ...
          <div style={{ borderRadius: "8px", overflow: "hidden", border: `1px solid ${C.border}` }}>
            ...
            >
              <div>
                ...
                >
                  {isHe
                    ? "נמצא בתוך קטגוריית הספורט"
                    : "Lives inside the sports category"}
                </div>
              </div>
              <Link
          ...
          >
            <div>
              ...
              <div style={{ fontSize: "13px", color: C.muted, lineHeight: 1.8 }}>
                {isHe
                  ? "פרסם מודעה תוך דקה, והקונים יפנו אליך ישירות."
                  : "Post a listing in a minute, and buyers will contact you directly."}
              </div>
            </div>
