...
// ── Types ─────────────────────────────────────────────────────────────────────
type MatchItem = {
  id: string; fifa_match_number: number;
  home_team_name: string | null; away_team_name: string | null;
  city: string; stadium: string; match_date: string; match_time: string; stage: string;
};
type ListingItem = {
  id: string; user_id: string; match_id: string; type: string; category: string;
  quantity: number; price: number; notes: string | null; status: string;
  expires_at: string | null; is_featured?: boolean;
  seated_together?: string | null; seats_block?: string | null;
  seats_row?: string | null; seats_numbers?: string | null;
  last_bumped_at?: string | null; first_published_at?: string | null;
  first_featured_at?: string | null;
};
type ProfileItem = {
  id: string; full_name: string | null; phone: string | null;
  country: string | null; city: string | null; created_at?: string | null;
};
type EnrichedListing = ListingItem & { profile: ProfileItem | null };

// ── Tokens ────────────────────────────────────────────────────────────────────
const C = {
  usa: "#1a3a6b", canada: "#e63946", mexico: "#006847",
  bg: "#f8f9fc", white: "#ffffff", border: "#e8edf5",
  text: "#0d1b3e", muted: "#64748b", hint: "#94a3b8", faint: "#cbd5e1",
  gold: "#d4a017", goldLight: "rgba(212,160,23,0.1)",
  green: "#22c55e", greenDark: "#065f46", greenLight: "rgba(0,104,71,0.08)",
};

// ── Countdown ─────────────────────────────────────────────────────────────────
function Countdown({ date, time, isHe }: { date: string; time: string; isHe: boolean }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
      <span style={{ fontSize: "10px", fontWeight: 600, color: C.hint, letterSpacing: "0.08em", textTransform: "uppercase" }}>
        {isHe ? "עד הקיקאוף" : "Kickoff in"}
      </span>
      <div style={{ display: "flex", gap: "4px" }}>
        {[{ v: t.d, l: isHe ? "י" : "d" }, { v: t.h, l: isHe ? "ש" : "h" }, { v: t.m, l: isHe ? "ד" : "m" }, { v: t.s, l: isHe ? "ש" : "s" }].map(({ v, l }) => (
          <div key={l} style={{ textAlign: "center", minWidth: "36px", background: C.bg, border: `1px solid ${C.border}`, borderRadius: "4px", padding: "5px 4px" }}>
            ...
            <div style={{ fontSize: "8px", fontWeight: 700, color: C.hint, letterSpacing: "0.1em", textTransform: "uppercase", marginTop: "2px" }}>{l}</div>
          </div>
        ))}
      </div>
    </div>
...
function TrustBadge({ profile, isHe }: { profile: ProfileItem | null; isHe: boolean }) {
  ...
  return (
    <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 7px", borderRadius: "3px", background: `${color}15`, color, border: `1px solid ${color}28`, letterSpacing: "0.05em", textTransform: "uppercase" }}>
      {verified ? "✓ " : ""}{label}
    </span>
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hov ? "0 8px 40px rgba(212,160,23,0.18), 0 2px 12px rgba(13,27,62,0.08)" : "0 2px 12px rgba(13,27,62,0.06)",
        transform: hov ? "translateY(-2px)" : "none",
      }}
    >
      {/* 3-color top bar */}
      <div style={{ height: "3px", background: `linear-gradient(90deg, ${C.usa}, ${C.gold}, ${C.mexico})` }} />
      ...
      <div style={{ padding: "18px 20px", position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px", flexWrap: "wrap", gap: "8px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
            ...
            {isBestValue && (
              <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 8px", borderRadius: "3px", background: C.greenLight, color: C.greenDark, border: `1px solid rgba(0,104,71,0.22)`, letterSpacing: "0.05em", textTransform: "uppercase" }}>
                🏷️ {isHe ? "הכי זול בקטגוריה" : "Best value in cat"}
              </span>
            )}
            <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 8px", borderRadius: "3px", background: C.greenLight, color: C.greenDark, border: `1px solid rgba(0,104,71,0.2)`, letterSpacing: "0.05em", textTransform: "uppercase" }}>
              {isHe ? "מכירה" : "Sell"}
            </span>
            <span style={{ fontSize: "9px", fontWeight: 600, padding: "2px 8px", borderRadius: "3px", background: "#f1f5f9", color: C.muted, border: `1px solid ${C.border}` }}>
              {item.category}
            </span>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: "10px", marginBottom: "10px" }}>
          ...
          {item.quantity > 1 && (
            <span style={{ fontSize: "13px", fontWeight: 700, color: C.gold }}>= ${item.price * item.quantity}</span>
          )}
        </div>
        ...
        {/* Seller + CTA */}
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: `linear-gradient(135deg, ${C.usa}, ${C.mexico})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: 700, color: "#fff", flexShrink: 0 }}>
    ...
    >
      ...
      <div style={{ padding: "15px 18px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "5px", marginBottom: "10px", flexWrap: "wrap" }}>
          {isBestValue && (
            <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 7px", borderRadius: "3px", background: C.greenLight, color: C.greenDark, border: `1px solid rgba(0,104,71,0.2)`, letterSpacing: "0.05em", textTransform: "uppercase" }}>
              🏷️ {isHe ? "הכי זול" : "Best value"}
            </span>
          )}
          {isNew && (
            <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 7px", borderRadius: "3px", background: "rgba(26,58,107,0.07)", color: C.usa, border: `1px solid rgba(26,58,107,0.18)`, letterSpacing: "0.05em", textTransform: "uppercase" }}>
              {isHe ? "חדש" : "New"}
            </span>
          )}
          <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 7px", borderRadius: "3px", textTransform: "uppercase", letterSpacing: "0.05em", background: item.type === "sell" ? C.greenLight : "rgba(26,58,107,0.07)", color: item.type === "sell" ? C.greenDark : C.usa, border: `1px solid ${item.type === "sell" ? "rgba(0,104,71,0.2)" : "rgba(26,58,107,0.15)"}` }}>
          ...
          {expiresSoon && hoursLeft !== null && (
            <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 7px", borderRadius: "3px", background: "rgba(230,57,70,0.07)", color: C.canada, border: `1px solid rgba(230,57,70,0.2)`, letterSpacing: "0.04em" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "16px", alignItems: "center" }}>
          <div>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: "8px", marginBottom: "6px" }}>
              <span style={{ fontFamily: "var(--font-syne,'Syne',sans-serif)", fontSize: "22px", fontWeight: 800, color: isBestValue ? C.mexico : C.text, letterSpacing: "-0.5px", lineHeight: 1 }}>
            ...
            {(item.seats_block || item.seats_row) && (
              <div style={{ display: "flex", gap: "5px", marginBottom: "6px", flexWrap: "wrap" }}>
                {[item.seats_block && `${isHe ? "בלוק" : "Blk"} ${item.seats_block}`, item.seats_row && `${isHe ? "שורה" : "Row"} ${item.seats_row}`, item.seated_together === "yes" && (isHe ? "✓ יחד" : "✓ Together")].filter(Boolean).map((v: any, i) => (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
              <div style={{ width: "24px", height: "24px", borderRadius: "50%", background: `linear-gradient(135deg, ${C.usa}80, ${C.mexico}80)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px", fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                {(item.profile?.full_name || "?")[0].toUpperCase()}
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px", alignItems: "flex-end" }}>
            {!viewerLoggedIn ? (
              <Link href="/auth" style={{ padding: "9px 16px", background: C.usa, color: "#fff", fontSize: "11px", fontWeight: 700, borderRadius: "4px", textDecoration: "none", whiteSpace: "nowrap" }}>
                {isHe ? "התחבר" : "Login"}
              </Link>
            ) : !viewerProfileComplete ? (
              <Link href="/complete-profile" style={{ padding: "9px 16px", background: C.usa, color: "#fff", fontSize: "11px", fontWeight: 700, borderRadius: "4px", textDecoration: "none", whiteSpace: "nowrap" }}>
                {isHe ? "השלם פרופיל" : "Complete profile"}
              </Link>
            ) : item.profile?.phone ? (
              <button onClick={() => onContact(item)}
                style={{ padding: "9px 16px", background: "#25D366", color: "#fff", fontSize: "11px", fontWeight: 700, border: "none", borderRadius: "4px", cursor: "pointer", whiteSpace: "nowrap" }}>
                WhatsApp →
              </button>
            ) : (
              <span style={{ fontSize: "10px", color: C.faint }}>{isHe ? "אין מספר" : "No number"}</span>
            )}
            <Link href={`/contact?type=report&listingId=${item.id}&matchId=${item.match_id}`}
              style={{ fontSize: "10px", color: C.faint, textDecoration: "none", textAlign: "center" }}>
              {isHe ? "דווח" : "Report"}
            </Link>
          </div>
        </div>
...
export default function MatchPage() {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`match-${matchId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "listings", filter: `match_id=eq.${matchId}` }, async (payload) => {
  ...
  async function loadData() {
    ...
    if (!matchData) return;
    setMatch(matchData as MatchItem);
    const { data: listingsData } = await supabase.from("listings").select("*").eq("match_id", matchId).eq("status", "active").is("archived_at", null).gt("expires_at", new Date().toISOString());
  ...
  return (
    <main style={{ minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "1.5rem 1.75rem" }}>
        ...
        {/* ── BACK BUTTON ── */}
        <Link href="/"
          style={{ display: "inline-flex", alignItems: "center", gap: "6px", fontSize: "12px", fontWeight: 600, color: C.hint, textDecoration: "none", marginBottom: "16px", letterSpacing: "0.06em", textTransform: "uppercase", transition: "color 150ms" }}
        ...
        <div className="fsu" style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: "10px", overflow: "hidden", marginBottom: "12px", boxShadow: "0 2px 12px rgba(13,27,62,0.06)" }}>
          ...
          <div style={{ padding: "22px 26px" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "20px", flexWrap: "wrap" }}>
              <div style={{ flex: 1 }}>
                {/* Stage + number */}
                <div style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: C.hint, marginBottom: "8px" }}>
                ...
                {/* Meta */}
                <div style={{ display: "flex", flexWrap: "wrap", gap: "14px", fontSize: "12px", color: C.muted, marginBottom: "16px" }}>
                ...
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  <button
                    onClick={() => { if (!viewerLoggedIn) { setShowLoginModal(true); return; } window.location.href = `/post-listing?matchId=${matchId}&type=sell`; }}
                    style={{ padding: "10px 22px", background: C.usa, color: "#fff", fontSize: "12px", fontWeight: 700, border: "none", borderRadius: "5px", cursor: "pointer", letterSpacing: "0.02em" }}>
                    + {isHe ? "מכור כרטיסים" : "Sell tickets"}
                  </button>
                  <button
                    onClick={() => { if (!viewerLoggedIn) { setShowLoginModal(true); return; } window.location.href = `/post-listing?matchId=${matchId}&type=buy`; }}
                    style={{ padding: "10px 22px", background: "transparent", color: C.usa, fontSize: "12px", fontWeight: 700, border: `1px solid ${C.usa}`, borderRadius: "5px", cursor: "pointer" }}>
                    + {isHe ? "חפש לקנות" : "Looking to buy"}
                  </button>
                </div>
              </div>

              {/* Right side: stats + countdown */}
        ...
        {/* ── FILTER BAR ── */}
        <div className="fsu fsu1" style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "11px 16px", marginBottom: "12px", display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap", boxShadow: "0 1px 4px rgba(13,27,62,0.04)" }}>
          <div style={{ display: "flex", gap: "4px" }}>
            {(["all", "sell", "buy"] as const).map((v) => (
              <button key={v} onClick={() => setTypeFilter(v)}
                style={{ padding: "5px 12px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", border: `1px solid ${typeFilter === v ? C.usa : C.border}`, color: typeFilter === v ? C.usa : C.hint, background: typeFilter === v ? "rgba(26,58,107,0.06)" : "transparent", borderRadius: "4px", cursor: "pointer", transition: "all 150ms" }}>
                {v === "all" ? (isHe ? "הכל" : "All") : v === "sell" ? `${isHe ? "מכירה" : "Sell"} (${sellCount})` : `${isHe ? "קנייה" : "Buy"} (${buyCount})`}
              </button>
            ))}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
            <span style={{ fontSize: "10px", color: C.hint, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>$</span>
            <input type="number" value={minPrice} onChange={(e) => setMinPrice(e.target.value)} placeholder={isHe ? "מינ׳" : "min"}
              style={{ width: "58px", padding: "5px 8px", border: `1px solid ${C.border}`, borderRadius: "4px", fontSize: "11px", background: C.bg, color: C.text, outline: "none" }} />
            <span style={{ fontSize: "10px", color: C.faint }}>—</span>
            <input type="number" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} placeholder={isHe ? "מקס׳" : "max"}
              style={{ width: "58px", padding: "5px 8px", border: `1px solid ${C.border}`, borderRadius: "4px", fontSize: "11px", background: C.bg, color: C.text, outline: "none" }} />
          </div>
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
            style={{ padding: "5px 10px", border: `1px solid ${C.border}`, borderRadius: "4px", fontSize: "11px", background: C.bg, color: C.text, outline: "none", cursor: "pointer", marginInlineStart: "auto" }}>
        ...
        <div className="fsu fsu2">
          {filtered.length === 0 ? (
            <div style={{ padding: "56px 24px", textAlign: "center", background: C.white, border: `1px solid ${C.border}`, borderRadius: "10px", boxShadow: "0 1px 4px rgba(13,27,62,0.04)" }}>
              ...
              <button onClick={() => { if (!viewerLoggedIn) { setShowLoginModal(true); return; } window.location.href = `/post-listing?matchId=${matchId}`; }}
                style={{ padding: "12px 28px", background: C.usa, color: "#fff", fontSize: "13px", fontWeight: 700, border: "none", borderRadius: "5px", cursor: "pointer" }}>
      ...
      {showLoginModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(13,27,62,0.5)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 99, padding: "1.5rem" }}
          ...
          <div style={{ background: "#fff", borderRadius: "12px", padding: "32px", maxWidth: "360px", width: "100%", textAlign: "center", boxShadow: "0 32px 80px rgba(13,27,62,0.2)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <Link href="/auth" style={{ display: "block", padding: "12px", background: C.usa, color: "#fff", borderRadius: "5px", fontSize: "13px", fontWeight: 700, textDecoration: "none" }}>
                {isHe ? "התחבר / הרשמה" : "Login / Sign up"}
              </Link>
              <button onClick={() => setShowLoginModal(false)}
                style={{ padding: "12px", background: "transparent", border: `1px solid ${C.border}`, borderRadius: "5px", fontSize: "12px", fontWeight: 500, color: C.muted, cursor: "pointer" }}>
