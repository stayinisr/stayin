...
function useCountdown(date: string, time: string) {
  ...
  useEffect(() => {
    const cleanTime = (time || "20:00").replace(/:\d\d$/, "").slice(0, 5) || "20:00";
...
function MatchCard({
  ...
  match: MatchItem; idx: number; sell: number; buy: number;
      ...
      <div style={{ padding: "20px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
            ...
            {hot && (
              <span style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" as const, padding: "2px 7px", borderRadius: "3px", background: "rgba(230,57,70,0.08)", color: C.canada, border: "1px solid rgba(230,57,70,0.2)" }}>
                🔥 Hot
              </span>
            )}
            {countdown && (
              <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 7px", borderRadius: "3px", background: "rgba(26,58,107,0.07)", color: C.usa, border: "1px solid rgba(26,58,107,0.15)" }}>
                {countdown}
              </span>
            )}
          </div>
          ...
          <button
            ...
            style={{ width: "26px", height: "26px", border: `1px solid ${saved ? "rgba(230,57,70,0.35)" : C.border}`, borderRadius: "3px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", color: saved ? C.canada : C.faint, background: saved ? "rgba(230,57,70,0.06)" : "transparent", cursor: "pointer", transition: "all 150ms", flexShrink: 0 }}
          >
            {saved ? "♥" : "♡"}
          </button>
        </div>
        ...
        <Link href={`/matches/${match.id}`} style={{ textDecoration: "none", display: "block", marginBottom: "7px" }}>
          <div style={{ fontFamily: "var(--font-dm,'DM Sans',sans-serif)", fontSize: "15px", fontWeight: 600, letterSpacing: "-0.1px", color: C.text, lineHeight: 1.3 }}>
            <img src={`https://flagcdn.com/w20/${teamCode(match.home_team_name)}.png`} style={{ height: "13px", borderRadius: "2px", verticalAlign: "middle", marginRight: "5px" }} />{match.home_team_name || "TBD"}
            <br />
            <span style={{ color: C.hint, fontWeight: 400 }}>vs </span>
            <img src={`https://flagcdn.com/w20/${teamCode(match.away_team_name)}.png`} style={{ height: "13px", borderRadius: "2px", verticalAlign: "middle", marginRight: "5px" }} />{match.away_team_name || "TBD"}
          </div>
        </Link>
        ...
        {sell === 0 && buy === 0 ? (
          <Link
            ...
            style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "9px 14px", border: `1px solid ${C.border}`, borderRadius: "3px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: C.usa, textDecoration: "none", transition: "all 150ms" }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "rgba(26,58,107,0.05)"; (e.currentTarget as HTMLElement).style.borderColor = C.usa; }}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontFamily: "var(--font-syne,'Syne',sans-serif)", fontSize: "14px", fontWeight: 800, color: priceRange ? C.usa : C.faint, letterSpacing: "-0.2px" }}>
              {priceRange || "—"}
            </span>
            <div style={{ display: "flex", gap: "4px" }}>
              <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 8px", borderRadius: "2px", textTransform: "uppercase" as const, letterSpacing: "0.04em", background: "rgba(26,58,107,0.07)", color: C.usa }}>
...
export default function Home() {
  ...
  useEffect(() => {
    try { setSaved(new Set(JSON.parse(localStorage.getItem("saved_matches") || "[]"))); } catch {}
    load();
    ...
    const ch = supabase.channel("home-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "listings" }, (p) => {
        ...
        if (p.eventType === "INSERT") {
          ...
          setListings((prev) => prev.map((l) => l.id === p.new.id ? p.new as ListingItem : l));
        }
      }).subscribe();
  ...
  async function load() {
    ...
    const [{ data: m }, { data: l }] = await Promise.all([
      supabase.from("matches").select("*").order("fifa_match_number", { ascending: true }),
      supabase.from("listings").select("id,match_id,price,type,status,expires_at,archived_at").eq("status", "active").is("archived_at", null),
    ]);
  ...
  const toggleSave = useCallback((id: string) => {
    setSaved((prev) => {
      ...
      if (next.has(id)) { next.delete(id); toast.show(isHe ? "הוסר מהמועדפים" : "Removed", "info"); }
      else { next.add(id); toast.success(isHe ? "נשמר ❤️" : "Saved ❤️"); }
  ...
  const availableStages = useMemo(() => {
    ...
    const nonGroups = STAGE_ORDER.filter((s) => found.some((f) => f.toLowerCase().includes(s.toLowerCase())));
    return hasGroups ? ["Group Stage", ...nonGroups] : nonGroups;
  }, [matches]);

  // For filtering — Group Stage chip filters all groups
  function matchesStage(match: MatchItem) {
    if (!activeStage) return true;
    if (activeStage === "Group Stage") return match.stage?.startsWith("Group") || match.stage === "Group Stage";
    return match.stage === activeStage;
  }

  function getSell(id: string)  { return listings.filter((l) => l.match_id === id && l.type === "sell" && isActive(l)).length; }
  function getBuy(id: string)   { return listings.filter((l) => l.match_id === id && l.type === "buy"  && isActive(l)).length; }
  function isHot(id: string)    { return listings.filter((l) => l.match_id === id && isActive(l)).length >= 10; }
  function getPrice(id: string) {
    const prices = listings.filter((l) => l.match_id === id && l.type === "sell" && isActive(l)).map((l) => Number(l.price)).filter(Boolean);
  ...
  const filtered = useMemo(() => {
    ...
    return matches.filter((m) => {
      ...
      if (q && ![m.fifa_match_number, m.home_team_name, m.away_team_name, m.stage, m.city, m.stadium].join(" ").toLowerCase().includes(q)) return false;
      if (mn !== null || mx !== null) {
        const prices = listings.filter((l) => l.match_id === m.id && l.type === "sell" && isActive(l)).map((l) => Number(l.price));
    ...
    <main style={{ background: C.bg, minHeight: "100vh", color: C.text }}>
      ...
      <div style={{ background: "transparent", borderBottom: `1px solid ${C.border}` }}>
        <div style={{ ...W, paddingTop: "44px", paddingBottom: "40px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "32px", alignItems: "center" }} className="hero-grid">
            ...
            <div>
              {/* Eyebrow */}
              <div style={{ fontSize: "11px", fontWeight: 800, letterSpacing: "0.24em", textTransform: "uppercase", color: C.usa, marginBottom: "16px" }}>
                STAY IN THE GAME
              </div>
              ...
              <p style={{ fontSize: "15px", fontWeight: 400, color: C.muted, lineHeight: 1.8, maxWidth: "420px", marginBottom: "28px", letterSpacing: isHe ? "0" : "0.01em", fontFamily: isHe ? "var(--font-he,'Heebo',sans-serif)" : "var(--font-dm,'DM Sans',sans-serif)" }}>
                {isHe
              ...
              <div className="hero-btns" style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
                <a href="#matches"
                  ...
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "transparent"; (e.currentTarget as HTMLElement).style.color = C.usa; }}
                >
                  {isHe ? "צפה במשחקים ↓" : "Browse matches ↓"}
                </a>
                {isLoggedIn
                  ? <Link href="/post-listing"
                      style={{ padding: "12px 22px", border: `1px solid ${C.border}`, color: C.muted, fontSize: "13px", fontWeight: 500, borderRadius: "4px", textDecoration: "none", background: C.white, transition: "border-color 150ms,color 150ms" }}
                  ...
                  : <Link href="/auth"
                      style={{ padding: "12px 22px", border: `1px solid ${C.border}`, color: C.muted, fontSize: "13px", fontWeight: 500, borderRadius: "4px", textDecoration: "none", background: C.white }}
                    >
            ...
            <div className="hero-stats" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1px", background: C.border, border: `1px solid ${C.border}`, borderRadius: "6px", overflow: "hidden" }}>
              {[
                ...
                <div key={i} style={{ background: "transparent", padding: "18px 14px", textAlign: "center" }}>
                  <div style={{ fontFamily: "var(--font-syne,'Syne',sans-serif)", fontSize: "22px", fontWeight: 800, color: s.color, letterSpacing: "-0.5px", display: "flex", alignItems: "center", justifyContent: "center", gap: "5px" }}>
                    {s.val}
                    {s.live && <span className="ld" style={{ width: "6px", height: "6px", borderRadius: "50%", background: pulse ? "#34d399" : "#22c55e", display: "inline-block", transition: "background 400ms" }} />}
                  </div>
      ...
      <div id="matches" style={{ ...W, paddingTop: "24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
          <span style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: C.hint, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", flex: 1, gap: "8px" }}>
            <div style={{ flex: 1, display: "flex", alignItems: "center", border: `1px solid ${C.border}`, borderRadius: "4px", background: C.white }}>
              ...
              <input
                ...
                style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: C.text, fontSize: "12px", padding: "10px 0", fontFamily: "var(--font-dm),var(--font-he),sans-serif" }}
              />
            </div>
            <div style={{ flex: 1, display: "flex", alignItems: "center", border: `1px solid ${C.border}`, borderRadius: "4px", background: C.white }}>
              <span style={{ padding: "0 10px", fontSize: "12px", color: C.faint }}>$</span>
            ...
            {(minPrice || maxPrice) && (
              <button
                ...
                style={{ padding: "0 12px", background: "none", border: `1px solid ${C.border}`, borderRadius: "4px", color: C.hint, cursor: "pointer", fontSize: "12px", whiteSpace: "nowrap" }}
              >
                ✕
              </button>
            )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "16px", flexWrap: "wrap" }}>
          {/* All chip */}
          <button
            onClick={() => { setActiveStage(null); setSavedOnly(false); }}
            style={{ padding: "5px 14px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", border: `1px solid ${!activeStage && !savedOnly ? C.usa : C.border}`, color: !activeStage && !savedOnly ? C.usa : C.hint, background: !activeStage && !savedOnly ? "rgba(26,58,107,0.05)" : C.white, cursor: "pointer", borderRadius: "3px", transition: "all 150ms", fontFamily: "var(--font-dm),sans-serif" }}
          >
            {isHe ? "הכל" : "All"}
          </button>
          ...
          {availableStages.map((stage) => (
            <button key={stage}
              onClick={() => { setActiveStage(activeStage === stage ? null : stage); setSavedOnly(false); }}
              style={{ padding: "5px 14px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", border: `1px solid ${activeStage === stage ? C.usa : C.border}`, color: activeStage === stage ? C.usa : C.hint, background: activeStage === stage ? "rgba(26,58,107,0.05)" : C.white, cursor: "pointer", borderRadius: "3px", transition: "all 150ms", whiteSpace: "nowrap", fontFamily: "var(--font-dm),sans-serif" }}
            >
              {stage}
            </button>
          ))}
          ...
          {/* Saved chip */}
          <button
            onClick={() => { setSavedOnly(!savedOnly); setActiveStage(null); }}
            style={{ padding: "5px 14px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", border: `1px solid ${savedOnly ? "rgba(230,57,70,0.4)" : C.border}`, color: savedOnly ? C.canada : C.hint, background: savedOnly ? "rgba(230,57,70,0.05)" : C.white, cursor: "pointer", borderRadius: "3px", transition: "all 150ms", fontFamily: "var(--font-dm),sans-serif" }}
          >
          ...
          <div style={{ marginInlineStart: "auto", display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: C.hint }}>
            <span className="ld" style={{ width: "5px", height: "5px", borderRadius: "50%", background: pulse ? "#34d399" : "#22c55e", display: "inline-block", transition: "background 400ms" }} />
            {isHe ? "מתעדכן חי" : "Live updates"}
            {!loading && (
              <span style={{ color: C.usa, fontWeight: 600 }}>
                · {filtered.length} {filtered.length === 1 ? t.matchSingleFound : t.matchesFound}
              </span>
            )}
          </div>
        </div>
        ...
        {loading ? (
            ...
            {[1,2,3,4].map((i) => (
              <div key={i} style={{ background: C.white, padding: "20px" }}>
                ...
                {[60, 120, 90, 50].map((w, j) => (
                  <div key={j} className="sk" style={{ height: "12px", width: `${w}%`, marginBottom: "12px" }} />
                ))}
              </div>
            ))}
          </div>
          ...
          <div style={{ padding: "60px 24px", textAlign: "center", background: C.white, border: `1px solid ${C.border}`, borderRadius: "6px" }}>
            ...
            <p style={{ fontSize: "14px", color: C.muted, marginBottom: "4px" }}>
              {savedOnly ? (isHe ? "אין משחקים שמורים עדיין" : "No saved matches yet") : t.noMatchesFound}
            </p>
            <p style={{ fontSize: "12px", color: C.hint }}>
              {savedOnly ? (isHe ? "לחץ על ♡ בכרטיס להוסיף" : "Tap ♡ on any card") : t.tryChangingFilters}
            </p>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: "1px", background: C.border, borderRadius: "6px", overflow: "hidden", border: `1px solid ${C.border}` }}>
            {filtered.map((match, i) => (
              <MatchCard key={match.id} match={match} idx={i}
                sell={getSell(match.id)} buy={getBuy(match.id)}
                priceRange={getPrice(match.id)} hot={isHot(match.id)}
                saved={saved.has(match.id)} onSave={toggleSave}
                isHe={isHe}
              />
            ))}
          </div>
      ...
      <div style={{ ...W, marginTop: "28px" }}>
        <div style={{ borderRadius: "6px", overflow: "hidden", border: `1px solid ${C.border}` }}>
          <div style={{ background: C.usa, padding: "16px 22px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontFamily: "var(--font-syne,'Syne',sans-serif)", fontSize: "13px", fontWeight: 700, color: "#fff" }}>FIFA World Cup 2026</div>
              <div style={{ fontSize: "11px", color: "rgba(255,255,255,.5)", marginTop: "2px", fontWeight: 300 }}>48 teams · 104 matches · 16 host cities</div>
            </div>
            <div style={{ fontSize: "11px", fontWeight: 700, color: "#fff", background: "rgba(255,255,255,.12)", padding: "5px 12px", borderRadius: "3px", border: "1px solid rgba(255,255,255,.15)", whiteSpace: "nowrap" }}>
              Jun 11 – Jul 19
            </div>
          </div>
      ...
      <div style={{ ...W, marginTop: "20px", paddingBottom: "52px" }}>
        <div style={{ padding: "24px", border: `1px solid ${C.border}`, borderRadius: "6px", background: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(8px)', display: "flex", alignItems: "center", justifyContent: "space-between", gap: "20px", flexWrap: "wrap" }}>
          ...
          <Link href={isLoggedIn ? "/post-listing" : "/auth"}
            style={{ padding: "12px 26px", background: C.usa, color: "#fff", fontSize: "13px", fontWeight: 700, borderRadius: "4px", textDecoration: "none", whiteSpace: "nowrap", letterSpacing: "0.02em", transition: "opacity 150ms", flexShrink: 0 }}
