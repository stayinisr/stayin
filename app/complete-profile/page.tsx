...
import AddToHomeCard from "../../components/AddToHomeCard";
...
export default function CompleteProfilePage() {
  ...
  const [showAddToHomeCard, setShowAddToHomeCard] = useState(false);
  ...
  useEffect(() => {
    setShowAddToHomeCard(canShowAddToHome("a2hs_after_signup_hidden_until"));
  }, []);

  const selectedCountry = useMemo(
    () => COUNTRIES.find((c) => c.country === country) || COUNTRIES[0],
    [country]
  );
  ...
  async function loadProfile() {
    ...
    if (data) {
      ...
      setCountry(data.country || "Israel");

      const saved = data.phone || "";
      const prefix =
        COUNTRIES.map((c) => c.code)
          .sort((a, b) => b.length - a.length)
          .find((p) => saved.startsWith(p)) || "+972";

      setCountryCode(prefix);
      setPhoneLocal(
        saved.startsWith(prefix)
          ? saved.slice(prefix.length).replace(/\D/g, "")
          : saved.replace(/\D/g, "")
      );
    }
  ...
  function nextStep() {
    if (step === 0 && !fullName.trim()) {
      toast.error(isHe ? "יש להזין שם מלא" : "Enter your full name");
      return;
    }

    if (step === 1 && !country.trim()) {
      toast.error(isHe ? "יש לבחור מדינה" : "Choose a country");
      return;
    }
  ...
  async function save() {
    if (!phoneLocal.trim()) {
      toast.error(isHe ? "יש להזין מספר טלפון" : "Enter your phone number");
      return;
    }
    ...
    if (error) {
      toast.error(isHe ? "שמירה נכשלה" : "Failed to save");
      return;
    }
  ...
  function handleDismissAddToHomeCard() {
    dismissAddToHome("a2hs_after_signup_hidden_until", 30);
    setShowAddToHomeCard(false);
    setOpenAddToHomeModal(false);
  }

  function handleOpenAddToHomeModal() {
    setOpenAddToHomeModal(true);
  }
      ...
      <main style={{ minHeight: "100vh" }}>
        ...
        >
          <style>{`
            ...
            .sk {
              background: linear-gradient(90deg,#f0f4f8 25%,#e8edf5 50%,#f0f4f8 75%);
          ...
          {[120, 80, 60].map((h, i) => (
            <div
              key={i}
              className="sk"
              style={{ height: `${h}px`, marginBottom: "12px" }}
            />
          ))}
        </div>
      </main>
    ...
    <main style={{ minHeight: "100vh" }}>
      ...
      >
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          >
            {isHe
              ? "נדרש כדי לפרסם מודעות וליצור קשר עם מוכרים."
              : "Required to post listings and contact sellers."}
          </p>
        </div>
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          >
            {[isHe ? "שם" : "Name", isHe ? "מדינה" : "Country", isHe ? "טלפון" : "Phone"].map(
              (l, i) => (
                <span
                  key={l}
                  style={{
          ...
          >
            <div
              style={{
                height: "100%",
                width: `${progressPct}%`,
                background: `linear-gradient(90deg, ${C.usa}, ${C.mexico})`,
                borderRadius: "4px",
        ...
        <div style={card}>
          ...
          {step >= 0 && (
            <div style={{ marginBottom: "18px" }}>
              ...
              <input
                ...
                style={inp}
                onFocus={(e) => (e.target.style.borderColor = C.usa)}
                onBlur={(e) => (e.target.style.borderColor = C.border)}
              />
              <p style={{ fontSize: "11px", color: C.faint, marginTop: "5px" }}>
                {isHe ? "יוצג למשתמשים אחרים" : "Shown to other users"}
              </p>
            </div>
          )}
          ...
          {step >= 1 && (
            <div style={{ marginBottom: "18px" }}>
              ...
              <input
                ...
                placeholder={isHe ? "בחר מדינה" : "Choose your country"}
                style={inp}
                onFocus={(e) => (e.target.style.borderColor = C.usa)}
                onBlur={(e) => (e.target.style.borderColor = C.border)}
              />
          ...
          {step >= 2 && (
            <div style={{ marginBottom: "18px" }}>
              ...
              <p style={{ fontSize: "11px", color: C.faint, marginTop: "5px" }}>
                {isHe ? "יישמר כ: " : "Saved as: "}
                <span style={{ color: C.text, fontWeight: 500 }}>{fullPhone}</span>
              </p>
            </div>
          )}
        ...
        {showAddToHomeCard && (
          <AddToHomeCard
            ...
            description={
              isHe
                ? "כך תוכל לחזור מהר למשחקים, למודעות ולפניות — ממש כמו אפליקציה."
                : "Quick access to matches, listings and messages — just like an app."
            }
            onOpen={handleOpenAddToHomeModal}
            onDismiss={handleDismissAddToHomeCard}
          />
        )}

        <AddToHomeModal
          open={openAddToHomeModal}
          onClose={handleDismissAddToHomeCard}
          title={isHe ? "שמור את Stayin במסך הבית" : "Save Stayin to your home screen"}
          description={
            isHe
              ? "אחרי ההרשמה יהיה לך יותר נוח לחזור לאתר בלחיצה אחת."
              : "After signing up, it will be easier to come back in one tap."
          }
        />
      </div>
    </main>
