...
const C = { usa: "#1a3a6b", canada: "#e63946", mexico: "#006847", border: "#e8edf5", text: "#0d1b3e", muted: "#64748b", hint: "#94a3b8", faint: "#cbd5e1", bg: "#f8f9fc", gold: "#d4a017" };

type MatchItem = { id: string; fifa_match_number: number; home_team_name: string | null; away_team_name: string | null; city: string; stadium: string; match_date: string; };
...
export default function PostListingPage() {
  ...
  useEffect(() => {
    if (!editId) { if (preMatchId) setMatchId(preMatchId); if (preType) setType(preType); }
  }, [preMatchId, preType, editId]);

  useEffect(() => { if (editId) loadEdit(editId); }, [editId]);

  async function fetchMatches() {
    setLoadingM(true);
    const { data } = await supabase.from("matches").select("id,fifa_match_number,home_team_name,away_team_name,city,stadium,match_date").order("fifa_match_number", { ascending: true });
  ...
  async function loadEdit(id: string) {
    ...
    setType(data.type); setMatchId(data.match_id); setCategory(data.category);
    setQuantity(data.quantity); setPrice(String(data.price)); setNotes(data.notes || "");
    setSeatedTogether(data.seated_together || "unknown"); setSeatsBlock(data.seats_block || "");
    setSeatsRow(data.seats_row || ""); setSeatsNums(data.seats_numbers || "");
  }
  ...
  async function handleSubmit() {
    ...
    const { data: profile } = await supabase.from("profiles").select("full_name,phone,country,last_post_at,is_premium").eq("id", user.id).maybeSingle();
    if (!profile?.full_name || !profile?.phone || !profile?.country) {
      setSubmitting(false); toast.error(t.saveProfileFirst); router.push("/complete-profile"); return;
    }
    ...
    const payload: Record<string, any> = { type, match_id: matchId, category, quantity, price: Number(price), notes: notes.trim() || null, seated_together: quantity > 1 ? seatedTogether : "unknown", seats_block: seatsBlock.trim() || null, seats_row: seatsRow.trim() || null, seats_numbers: seatsNums.trim() || null, status: "active", expires_at: exp };
    ...
    if (editId) {
      ...
      toast.success(t.listingUpdated); router.push(`/matches/${matchId}`); return;
    }
    ...
    if (!profile.is_premium && (active || []).length >= 10) { setSubmitting(false); toast.error(t.limitReached10); return; }
    const { data: dup } = await supabase.from("listings").select("id").eq("user_id", user.id).eq("match_id", matchId).eq("type", type).eq("category", category).is("archived_at", null).maybeSingle();
  ...
  const card: React.CSSProperties = { background: "rgba(255,255,255,0.88)", border: `1px solid ${C.border}`, borderRadius: "10px", backdropFilter: "blur(12px)", boxShadow: "0 2px 16px rgba(13,27,62,0.06)", padding: "22px" };
  ...
  return (
    <main style={{ minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "1.75rem" }}>
        ...
        <Link href={matchId ? `/matches/${matchId}` : "/"} style={{ display: "inline-flex", alignItems: "center", gap: "5px", fontSize: "11px", fontWeight: 600, color: C.hint, textDecoration: "none", marginBottom: "20px", letterSpacing: "0.06em", textTransform: "uppercase" }}>
          ← {isHe ? "חזרה" : "Back"}
        </Link>
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <h1 style={{ fontFamily: "var(--font-syne,'Syne',sans-serif)", fontSize: "clamp(22px,3.5vw,34px)", fontWeight: 800, letterSpacing: "-0.5px", color: C.text, lineHeight: 1.1 }}>
            {editId ? (isHe ? "עריכת מודעה" : "Edit listing") : type === "sell" ? (isHe ? "מכירת כרטיסים" : "Sell tickets") : (isHe ? "חיפוש כרטיסים" : "Looking to buy")}
          </h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: "16px", alignItems: "start" }}>
          ...
          <div style={card}>
            <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: C.border, borderRadius: "6px", overflow: "hidden" }}>
                  {["sell", "buy"].map(v => (
                    <button key={v} onClick={() => setType(v)}
                      style={{ padding: "11px", fontSize: "12px", fontWeight: 700, border: "none", cursor: "pointer", transition: "all 150ms", background: type === v ? (v === "sell" ? C.mexico : C.usa) : "rgba(255,255,255,0.9)", color: type === v ? "#fff" : C.hint }}>
              ...
              <div>
                ...
                <select value={matchId} onChange={e => setMatchId(e.target.value)} disabled={loadingM} style={{ ...inp, cursor: "pointer" }}>
                  <option value="">{loadingM ? t.loading : t.selectMatch}</option>
                  {matches.map(m => (
                    <option key={m.id} value={m.id}>Match {m.fifa_match_number} · {m.home_team_name || "TBD"} vs {m.away_team_name || "TBD"} · {m.city}</option>
                  ))}
                </select>
              </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "6px" }}>
                  {["Category 1", "Category 2", "Category 3", "Category 4"].map(c => (
                    <button key={c} onClick={() => setCategory(c)}
                      style={{ padding: "9px 4px", fontSize: "11px", fontWeight: 600, border: `1px solid ${category === c ? C.usa : C.border}`, borderRadius: "6px", background: category === c ? "rgba(26,58,107,0.07)" : "rgba(255,255,255,0.9)", color: category === c ? C.usa : C.muted, cursor: "pointer", transition: "all 150ms" }}>
              ...
              <div>
                ...
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", insetInlineStart: "14px", top: "50%", transform: "translateY(-50%)", fontSize: "13px", color: C.hint, pointerEvents: "none" }}>$</span>
              ...
              <div>
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} placeholder={t.addUsefulDetails}
                  style={{ ...inp, resize: "vertical", lineHeight: 1.7 }} />
                <p style={{ fontSize: "10px", color: C.faint, marginTop: "5px" }}>{isHe ? "הערות אלה יופיעו במודעה שלך" : "These notes will appear on your listing"}</p>
              </div>

              <button onClick={handleSubmit} disabled={submitting}
                style={{ padding: "13px", background: C.usa, color: "#fff", fontSize: "13px", fontWeight: 700, border: "none", borderRadius: "6px", cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? 0.7 : 1, letterSpacing: "0.02em" }}>
          ...
          <div style={{ position: "sticky", top: "20px" }}>
            <div style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: C.hint, marginBottom: "10px" }}>
            ...
            <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: "10px", overflow: "hidden", boxShadow: "0 2px 12px rgba(13,27,62,0.06)" }}>
              ...
              <div style={{ padding: "16px" }}>
                ...
                <div style={{ display: "flex", gap: "5px", marginBottom: "10px", flexWrap: "wrap" }}>
                  <span style={{ fontSize: "9px", fontWeight: 700, padding: "2px 7px", borderRadius: "3px", textTransform: "uppercase", letterSpacing: "0.05em", background: type === "sell" ? "rgba(0,104,71,0.08)" : "rgba(26,58,107,0.07)", color: type === "sell" ? C.mexico : C.usa, border: `1px solid ${type === "sell" ? "rgba(0,104,71,0.2)" : "rgba(26,58,107,0.15)"}` }}>
                    {type === "sell" ? (isHe ? "מכירה" : "Sell") : (isHe ? "קנייה" : "Buy")}
                  </span>
                  <span style={{ fontSize: "9px", fontWeight: 600, padding: "2px 7px", borderRadius: "3px", background: "#f1f5f9", color: C.hint, border: `1px solid ${C.border}` }}>{category}</span>
                </div>
                ...
                <div style={{ fontFamily: "var(--font-syne,'Syne',sans-serif)", fontSize: price ? "24px" : "18px", fontWeight: 800, color: price ? C.text : C.faint, letterSpacing: "-0.5px", marginBottom: "6px" }}>
                  ...
                  {price && quantity > 1 && <span style={{ fontSize: "12px", color: C.hint, fontWeight: 400, marginInlineStart: "8px" }}>× {quantity} = ${Number(price) * quantity}</span>}
                </div>
                ...
                {/* Match */}
                {selectedMatch ? (
                  <div style={{ fontSize: "11px", color: C.hint, paddingTop: "10px", borderTop: `1px solid ${C.border}` }}>
                    ⚽ Match {selectedMatch.fifa_match_number} · {selectedMatch.home_team_name || "TBD"} vs {selectedMatch.away_team_name || "TBD"}
                    <br />{selectedMatch.city} · {selectedMatch.match_date}
                  </div>
                ) : (
            ...
            {/* Info box */}
            <div style={{ marginTop: "12px", padding: "12px 14px", background: "rgba(26,58,107,0.04)", border: `1px solid rgba(26,58,107,0.1)`, borderRadius: "8px" }}>
